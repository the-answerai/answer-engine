...
export function TagsPage() {
  ...
  async function confirmDelete() { if (!deleting) return; await remove.mutateAsync(deleting.id); setDeleting(null); }
  return <section className="workspace-page"><PageHeader eyebrow="FIG. 03 / TAXONOMY" title="Tags" description="Build a reusable taxonomy and assign it across your local content." actions={<Button onClick={() => setEditing('new')}>Create tag</Button>} />
    ...
    <TagForm key={editing === 'new' ? 'new' : editing?.id ?? 'closed'} open={Boolean(editing)} tag={editing === 'new' ? undefined : editing ?? undefined} tags={tags.data ?? []} busy={create.isPending || update.isPending} onClose={() => setEditing(null)} onSubmit={async (input) => { if (editing === 'new') await create.mutateAsync(input); else if (editing) await update.mutateAsync({ id: editing.id, input }); setEditing(null); }} />
    <ConfirmDialog open={Boolean(deleting)} title="Delete tag?" busy={remove.isPending} onClose={() => setDeleting(null)} onConfirm={() => void confirmDelete()}>The tag “{deleting?.label}” will be removed from active taxonomy and content assignments.</ConfirmDialog>
  </section>;
}

function TagForm({ open, tag, tags, busy, onClose, onSubmit }: { open: boolean; tag?: Tag; tags: Tag[]; busy: boolean; onClose(): void; onSubmit(input: { label: string; slug: string; description: string | null; category: string | null; parentId: string | null; color: string | null; metadata: Record<string, unknown> }): Promise<void> }) {
