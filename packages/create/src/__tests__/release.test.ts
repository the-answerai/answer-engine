...
describe('release manifest verification', () => {
  it('keeps the source manifest non-runnable until the release builder injects verified inputs', () => {
    ...
    expect(template.version).toBe('1.1.2');
    expect(template.tag).toBe('v1.1.2');
    expect(template.promptUrl).toContain('/v1.1.2/INSTALL_AGENT.md');
  ...
  it('rejects version, mutable URL, and mutable image mismatches', () => {
    ...
    expect(() => verifyReleaseManifest({ ...manifest, promptUrl: manifest.promptUrl.replace('/v1.1.2/', '/master/') }))
    ...
    expect(() => verifyReleaseManifest({
      ...manifest,
      images: { ...manifest.images, answerEngine: 'ghcr.io/the-answerai/answer-engine:1.1.2' },
  ...
  it('rejects mutable lifecycle image references', () => {
    ...
    expect(() => assertImmutableImageReference('ghcr.io/the-answerai/answer-engine:1.1.2'))
