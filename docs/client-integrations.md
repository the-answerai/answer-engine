...
| ChatGPT Desktop Chat/Work | Remote service | No direct access | Remote MCP only | Unavailable for this local install |
| ChatGPT Work/web | Remote service | No direct access | Remote MCP only, subject to plan/admin policy | Unavailable for this local install |
...
| Cowork local session | Local, policy-dependent | Yes when local MCP is allowed | Dual-host plugin | Restart and guided real recall |
...
OpenAI documents that ChatGPT custom apps connect to remote MCP servers rather
than directly to localhost. Anthropic documents that remote connectors originate
from its cloud and that remote Cowork sessions do not run local MCP servers.
Local Cowork can use plugin MCP only when desktop and administrator policy allow
it. Sources checked 2026-08-14:
...
- `~/.agents/plugins/marketplace.json` and
  `~/.agents/plugins/plugins/answer-engine` for Codex.
- `~/.codex/config.toml` for Codex stdio MCP.
- `~/.claude/skills/answer-engine` and `~/.claude.json` for Claude Code.
...
it never stores the API key. The MCP and CLI files that require the key are mode
`0600`. Re-running a matching plan is byte-stable, and drift is rejected instead
of overwritten.
...
cannot mark those clients complete; rerun interactively or remove them from the
selection.
